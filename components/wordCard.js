...
export default function WordCard({ x, pos, selectedCluster, setSelectedWord, setPopupOpen, hoveredPair, highlightedWords, editModeToggle, setWordToHighlight, isProd }) {
    ...
    return <div key={x.id  } style={{ left: `${pos[x.id] || -1000}px` }}
        ...
        className={`word-card-${x.id} word-card-individual  absolute min-w-24 max-w-24 min-h-16  lg:min-w-32 lg:max-w-32 lg:min-h-20 z-10 text-center  justify-center rounded-lg flex flex-col transition-all duration-500 ${langColors[x.lang][0]} hover:shadow-lg ${hoveredPair.includes(x.id) ? "shadow border-gray-500 border-4" : ""} ${highlightedWords.includes(x.id) ? "" : "!bg-gray-100 opacity-10 dark:!bg-gray-500 dark:!opacity-10 "
