...
export default function Legend({languages, isHiddenOnMobile}) {
    ...
    return <div key = {"legend"}  className={`max-h-56  overflow-auto mb-0 lg:m-10 right-0 top-0 m-2 z-40 h-fit border border-black bg-slate-300 rounded grid text-sm overflow-x-hidden dark:bg-gray-400 dark:text-slate-800 ${isHiddenOnMobile ? "w-0 lg:w-auto overflow-hidden lg:overflow-auto border-0 lg:border": ""}`} >
